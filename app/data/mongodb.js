...
exports.findLastLog = function(sorting,callback){
  ...
  collection.find({}, {"sort" : {"ts":-1},"limit":1}, function(err, cursor) {
      ...
      if (err) throw err;
		...
		findMinLastFrom(doc.ts.getTime() - 86400000, "T1", function(err,mint1){
			doc.T1MIN = mint1;
			findMaxLastFrom(doc.ts.getTime() - 86400000, "T1", function(err,maxt1){
				doc.T1MAX = maxt1;

				findMinLastFrom(doc.ts.getTime() - 86400000, "H1", function(err,minh1){
					doc.H1MIN = minh1;

					findMaxLastFrom(doc.ts.getTime() - 86400000, "H1", function(err,maxh1){
						doc.H1MAX = maxh1;

						findMinLastFrom(doc.ts.getTime() - 86400000, "PRESS", function(err,minpress){
							doc.PRESSMIN = minpress;

							findMaxLastFrom(doc.ts.getTime() - 86400000, "PRESS", function(err,maxpress){
								doc.PRESSMAX = maxpress;

									findMinLastFrom(doc.ts.getTime() - 86400000, "WC", function(err,minwc){
										doc.WCMIN = minwc;
...
findLastFrom = function(startFrom,fieldName, callback){
	...
	var q = {"ts" : {"$gte": new Date(startFrom)}};
